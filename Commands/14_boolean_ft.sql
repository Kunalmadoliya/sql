...
CREATE TABLE boolean_text_example (
    id INT PRIMARY KEY,
    content TEXT
);

INSERT INTO boolean_text_example (id, content) VALUES
...
create fulltext index boolean_table on boolean_text_example(content)

select * from boolean_text_example
where match content against("+large queries" in boolean mode)
...
select * from boolean_text_example
where match content against("+large +(>enabling)" in boolean mode)
